...
if ( ! class_exists( 'Yuki_Particles_Extension' ) ) {

	class Yuki_Particles_Extension {

		use Yuki_Particles_Effect;
		...
		public function __construct() {
			...
			add_filter( 'yuki_header_row_css', [ $this, 'header_row_particles_css' ], 10, 2 );
			add_filter( 'yuki_filter_dynamic_css', [ $this, 'site_background_particles_css' ] );
			add_filter( 'yuki_filter_dynamic_css', [ $this, 'featured_image_particles_css' ] );

			// Inject particle effect controls
			add_filter( 'yuki_header_row_style_controls', [ $this, 'header_row_particle_effect_controls' ], 11, 3 );
//			add_filter( 'yuki_background_controls', [ $this, 'background_particle_effect_controls' ] );
			add_filter( 'yuki_post_behind_featured_image_controls', [
				$this,
				'post_featured_image_particle_effect_controls'
			] );
			add_filter( 'yuki_page_behind_featured_image_controls', [
				$this,
				'page_featured_image_particle_effect_controls'
			] );

			// Render particles canvas
			add_action( 'yuki_start_header_row', [ $this, 'render_header_row_particles_canvas' ], 11 );
//			add_action( 'yuki_action_before', [ $this, 'render_background_particles_canvas' ] );
//			add_action( 'wp_body_open', [ $this, 'render_body_particles_canvas' ] );
			add_action( 'yuki_before_render_featured_image', [ $this, 'render_featured_image_particles_canvas' ] );

			// Preset effects
			$this->particles_presets = apply_filters( 'yuki_particles_effect_presets', [
		...
		public function header_row_particles_css( $css, $id ) {
			return array_merge( $css, $this->getParticlesCss( 'yuki_header_' . $id . '_row_' ) );
		}

		public function site_background_particles_css( $css ) {
			return array_merge( $css, $this->getParticlesCss(
				'yuki_site_background',
				'.yuki_site_background_particles_canvas,.yuki_site_body_particles'
			) );
		}
		...
		public function render_header_row_particles_canvas( $id ) {
			$this->renderParticlesCanvas( 'yuki_header_' . $id . '_row_' );
		}

		public function render_background_particles_canvas() {
			if ( ! CZ::checked( 'yuki_enable_site_wrap' ) ) {
				$this->renderParticlesCanvas( 'yuki_site_background' );

				return;
			}

			$scope = CZ::get( 'yuki_site_background_particles_scope' );
		...
		public function render_body_particles_canvas() {
			...
			if ( isset( $scope['site-body'] ) && $scope['site-body'] === 'yes' ) {
				$this->renderParticlesCanvas( 'yuki_site_background', 'yuki_site_body_particles' );
			}
		}
		...
		public function background_particle_effect_controls( $controls ) {
			return array_merge( $controls, [
				( new Section( 'yuki_site_background_enable_particles' ) )
					->setLabel( __( 'Background Particles Effect', 'yuki' ) )
					->enableSwitch( false )
					->setControls( $this->getParticleEffectControls( 'yuki_site_background', [ 'enable' ] ) )
				,
			] );
		}
		...
		public function post_featured_image_particle_effect_controls( $controls ) {
			return array_merge(
				$controls,
				[ ( new Separator() ) ],
				$this->getParticleEffectControls( "yuki_post_featured_image" ),
				[ ( new Separator() ) ]
			);
		}
		...
		public function page_featured_image_particle_effect_controls( $controls ) {
			return array_merge(
				$controls,
				[ ( new Separator() ) ],
				$this->getParticleEffectControls( "yuki_page_featured_image" ),
				[ ( new Separator() ) ]
			);
		}
	}
